...
`runPi(campaign, options)` creates one fresh Pi `runAgentLoop`. The application supplies a real Pi model, a `Models.streamSimple` implementation, label, system prompt, prompt, optional candidate hash, optional tools, and optional abort signal.
