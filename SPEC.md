...
`runPi` creates one typed `elenx.pi.run` span and one standard Pi `pi.ai.request` child for every logical provider operation, including continuations after tool results. With `continueOnLength` set on the call, a response that stops at the provider's per-response output limit or with a provider-retryable error re-enters the loop at most that many times, carrying the full transcript and a fixed continuation prompt; overflow-shaped length stops, non-retryable errors, and aborts always terminate. Each continuation is an ordinary logical provider operation with its own span and request checkpoint, and the continuation prompts are part of the recorded transcript. Provider adapters may retry an operation without exposing each wire attempt, and each request loop is bounded to thirty-two turns so repeated in-loop retries cannot spend without limit. The settled span tree is stored inside that call's returned JSON, so its label, optional candidate, and optional requested reasoning level supply the reason and configuration for each operation without adding a telemetry table or stored roll-up. Request leaves carry the Pi schema's provider, requested and served models, API, response, stop reason, usage, cache, Pi model-price cost estimate, HTTP-status, and error fields when available. Measured requests also carry Pi's per-bucket cost split (fresh input, output, cache read, cache write) as elenx-recorded `pi.ai.usage.cost_*` attributes; `derivePiSpend` surfaces it as `costBreakdownUsd` on each measured usage and on totals whose every measured request reports it.
