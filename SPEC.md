...
An optional `submissionGate: {completeArgument, reserveTokens}` freezes a stopping rule for the call's sole terminal tool. Pi's pre-tool hook declines a submission unless its named boolean argument is true or the context estimate reaches Pi's usable context limit minus the reserve. Declined attempts remain in the native transcript without executing the audited tool. Pi's native steering continues unfinished text and length-truncated responses in the same context. Native terminal-tool results end recorded executions, including errors. Schema validation rejected before the audited tool-call record returns feedback for correction in the same conversation. Pi's native context estimator uses the latest applicable usage plus estimated trailing messages. Pi's native output-cap helper preserves finalization space when honored by the provider. The gate replaces the ordinary aggregate-turn and length-continuation bounds, while transient-error recovery, cancellation, and overflow handling remain bounded as described below. Exhaustion without a terminal submission is a failed call. This gate controls handoff and grants no mathematical verification status.
