...
An optional `submissionGate: {completeArgument, reserveTokens?, contextBudgetTokens?}` freezes a stopping rule for the call's sole terminal tool. The context budget caps total context at the smaller of `contextBudgetTokens` and the model's context window; omitting it uses the model's window. The frozen model profile retains the actual model capacity. The reserve defaults to the model's maximum output tokens. Pi's pre-tool hook declines a submission unless its named boolean argument is true or the context estimate reaches the budget minus Pi's native safety margin and the reserve. Declined attempts remain in the native transcript without executing the audited tool. Pi's native steering continues unfinished text and length-truncated responses in the same context. Native terminal-tool results end recorded executions, including errors. Schema validation rejected before the audited tool-call record returns feedback for correction in the same conversation. Pi's native context estimator uses the latest applicable usage plus estimated trailing messages. Before the threshold, Pi's native output-cap helper preserves the reserve; at the threshold it releases that space for finalization. A budget that leaves no usable context after the safety margin and reserve is rejected. Providers must honor output caps for the allocation to hold. The gate replaces the ordinary aggregate-turn and length-continuation bounds, while transient-error recovery, cancellation, and overflow handling remain bounded as described below. Exhaustion without a terminal submission is a failed call. This gate controls handoff and grants no mathematical verification status.
