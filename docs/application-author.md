...
`piRequestAttempts(campaign.records(), call)` returns the JSON-semantic payload exposed by Pi's final pre-send hook for each provider operation in a Pi call, together with its completed or unsettled state. These payloads can contain full prompts and attached source text. Built-in adapters keep credentials and HTTP headers outside them. An absent outer call result leaves the provider outcome unknown even when its request checkpoint completed. Custom adapters must invoke the hook exactly once before dispatch and must never put credentials or tokens in its payload.
...
Use `openCampaign(path)` only after the prior writer or coordinator has terminated or closed, then derive the next application action from `campaign.records()`. Close every handle in `finally`; copying an open database is unsupported. Calls and tool calls without matching results have unknown outcomes and are not automatically replayable. Use `openReader(path)` for read-only inspection. A reader refuses a hot rollback journal because recovery would mutate the artifact, so reopen and close it once with the matching package revision before copying or reading it elsewhere. Elenx rejects WAL-format headers and `-wal` or `-shm` sidecars before SQLite opens the artifact; preserve them and checkpoint or convert only an operator-controlled copy with trusted SQLite.
