...
`run` creates or resumes the campaign. The declared task and settings must match exactly on every invocation. The execution contract is schema 4 with application `elenx-solve`, protocol `workflow`, and run arguments `task`, `campaign`, and `settings`.
...
3. Inspect `elenx-solve contract`. Confirm schema 4, application `elenx-solve`, protocol `workflow`, and the three run arguments.
4. Run `bun run check` in Elenx Lab. Confirm the single author manifest, schema-4 contract freeze, old-shape rejection, task-based debug, inspection-derived worker results, recovery, immutable attempt validation, Nomad HCL validation, and CLI help.
