...
| explorer | The role that writes note texts for one objective, choosing the method. |
...
| objective | The explorer's goal for one turn. The first objective is the problem. It says what the completion criteria still need, which verified notes can be built on, and which approaches are dead or have left their gap open; it never prescribes a method, construction, or proof plan, which the explorer chooses. The explorer chooses whether to persist with an unfinished approach. Abandonment policies belong in the experimental explorer guidance, not the shared coordinator. |
