...
| coordinator strategy | The COORD-1 experiment's frozen `coordinatorStrategy` setting. `free` uses the released coordinator prompt. `repertoire` adds the five advisory choices. Only this experimental revision supports the setting, under workflow schema 25. |
