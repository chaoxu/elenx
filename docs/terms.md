...
| objective | The explorer's goal for one turn. The first objective is the problem. It goes to what the completion criteria still need; a note that records only an approach or names a gap without closing it does not set it, and when later notes leave a named gap open it asks for a different construction or reduction rather than the same assertion again, saying what to abandon and why. |
