...
The completion field and reserve are frozen in the Pi request. The fold matches the exact prompt and gate. Pi owns steering and terminal-tool control. Continuations reuse the same messages and transport session without compaction, a new Explorer root, or a coordinator call. Normal mathematical checks apply after handoff. Provider errors, cancellation, and exhaustion without a valid submission remain operational outcomes.
