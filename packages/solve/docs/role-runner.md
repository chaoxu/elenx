...
When settings enable `explorerContinuation`, the Explorer's result schema also requires `solution: boolean`. A true value permits early submission of a claimed complete solution. Otherwise Pi's submission gate declines the attempt until its estimated context threshold is reached. The declined tool call never executes the audited submission tool, so the journal still has exactly one executed submission for a successful Explorer call. Its earlier attempts and feedback stay in the native transcript and later request checkpoints. Plain-text completion also receives a follow-up in the same context. Ordinary mode retains its original schema and stopping behavior.

The gate's tool, completion field, and reserve are frozen in the Pi request. The fold matches the enabled prompt and gate against the recorded model profile. Continuations reuse the same messages and transport session without compaction, a new Explorer root, or a coordinator call. Normal mathematical checks apply after handoff. Provider errors, cancellation, and exhaustion without a valid submission remain operational outcomes.
...
Cozo derives support closure through the single query in `support.ts`. Explorer support selection, verifier-input validation, verification-window calculation, individual verifier prompts, and accepted export all use it. TypeScript checks duplicate IDs, missing notes, and the requirement that support precedes its note, then sorts the result in numeric ID order. The Cozo Node binding is asynchronous, so verifier-input validation and verifier prompt construction await that query internally. Workflow schema 28 carries the optional Explorer continuation policy, external verification on notes, and durable submission boundaries.
