...
`CoordinatorInput` contains the task and every note, including the new notes that have no summary yet. `CoordinatorResult` files each of those notes with a summary, sets the next objective and the support notes the explorer reads in full, and optionally verifies one note against the support it declares. A note may be verified only after every note in its support is verified, so an accepted note's closure is verified by induction; a note is verified when one verification passed every verifier but requirements.
...
`inspect` reports the task, current phase, notes with verdicts, every role call once with its state and submission, telemetry-derived spend, and with `--include-requests` the exact requests. A terminal campaign adds `result`, derived from the journal. Each verifier call carries its verifier name and its candidate.
