...
Curator ingest writes a short summary and a proposition-only statement proposal for every finding. Existing truth-establishing calls certify statement form, and proof audit also certifies fidelity, without adding a phase or role. Proof and support stay in the exact text. Serve sees compact dependency, verification, failure, strategy, and size metadata while full texts remain behind explicit expansion. One append-only re-triage may supersede a stuck plan. Every non-`PASS` candidate follows the same failure path and suppresses unchanged bytes; verdict labels and reasons remain evidence rather than separate workflows. Context and explorer-turn admission are deterministic terminal controls.
