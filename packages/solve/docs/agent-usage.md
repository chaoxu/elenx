...
Workflow schema 36 drains the requested verification batches before another Explorer turn or the cap. The optional `explorerContinuation` setting remains available, and a gated submission rejected by schema validation can be corrected in the same call. It is off by default. With it enabled, every valid `submit_notes` call saves new notes and its receipt carries the actual result `{noteIds}` only, while Explorer continues in the same call until it claims a complete solution or reaches its estimated context threshold. Every nonterminal submission, including an empty `notes` array, is followed after the receipt by a fresh user message exactly `Keep trying, you can do it.`. At the threshold the call finalizes; an empty terminal submission gives the coordinator all saved notes and its existing `emptySubmission: true` notice asking for a different promising approach. Explorer chooses its route, and the message requires no separate planning submission. The generic kernel gate's `emptyArgument` remains available to other callers but is not enabled for Explorer. Structured support is validated without scanning note text; mathematical verification checks for undeclared dependencies. Inspection includes these notes before handoff and after interruption. A fresh retry receives the saved texts with stable IDs and the same guidance. Gated `incomplete.max_messages` recovery receives the same fresh user feedback without changing `maxRecoveries` accounting; other transient retries retain their behavior. See [Explorer continuation](../README.md#explorer-continuation) for headroom and comparison semantics. `explorerGuidance` remains per-turn advice, and inconclusive verification returns its report to Explorer. Preserve older journals with the exact implementation that wrote them. The updated solver deliberately refuses to replay them against changed prompts. Start a fresh campaign to use the new role contract.
