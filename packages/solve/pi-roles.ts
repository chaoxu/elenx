...
export function coordinatorCall(
  ...
  return {
    ...
    system: [
      ...
      "Then set the next objective for the explorer and choose its support: the notes it must read in full. The explorer sees every note's summary and verdicts and only the support notes' texts. A dead note may be read in full so that a new note removes its defect, but it cannot be built on. Never ask the explorer to check, polish, or restate a verified note; the next objective goes to what the completion criteria still need. A note that records only an approach, or names a gap without closing it, does not set the next objective. When the notes written after a note leave the gap it names still open, the next objective asks for a different construction or a different reduction of the problem rather than the same assertion again, and says what to abandon and why.",
