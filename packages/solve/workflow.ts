...
function journalVerdicts(
  records: readonly Entry[],
): readonly { readonly seq: EntryId; readonly verdict: Verdict }[] {
  const calls = new Map(
    records
      .filter((entry) => entry.kind === "call")
      .map((entry) => [entry.seq, entry]),
  );
  const verdicts: { seq: EntryId; verdict: Verdict }[] = [];
  for (const entry of records) {
    ...
    if (!parsed.success) throw new Error(`malformed verdict ${entry.seq}`);
    verdicts.push({ seq: entry.seq, verdict: parsed.data });
  }
  return verdicts;
}
...
export function deriveWorkflow(reader: Reader): WorkflowSnapshot {
  ...
  for (let turn = 1; turn <= config.settings.maxExplorerTurns; turn += 1) {
    ...
    const coordinated = settledCall(
      ...
      "coordinator",
      coordinatorRequest,
      coordinatorResultFor(coordinatorRequest.notes),
    );
    ...
    if (candidate === undefined) {
      throw new Error("verifier call is not bound to a candidate");
    }
