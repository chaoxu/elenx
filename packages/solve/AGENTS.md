...
Keep execution-contract schema version 3 while this contract remains unchanged. Standalone role commands exercise the same typed boundaries and must not become a second campaign mode. Elenx Lab must derive a worker result from `inspect.result`, not trust solver stdout as a second authority.
