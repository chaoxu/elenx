...
The current workflow declaration schema is 31 and execution-contract schema is 9. Notes may carry external `verification`, and an accepted result may have zero Explorer turns when a submitted proof passes all normal checks. Bump the workflow declaration schema whenever a role prompt or journal shape changes, and the execution contract when report shapes change. Standalone role commands exercise the same typed boundaries and are not a second workflow. Elenx Lab must derive a worker result from `inspect.result`, not trust solver stdout as a second authority.

`explorerContinuation` is an optional settings toggle, off when absent or false. Only the enabled Explorer uses the submission gate and `solution` field. Its total-context budget defaults to 400,000 tokens, bounded by model capacity, and `explorerContextBudgetTokens` may override it. Reserve the model's maximum output tokens for finalization. A claimed complete solution may hand off early, while partial work continues in the same call until its estimated context threshold. Declined attempts remain in the transcript and never enter the note graph. Mathematical verification retains its authority. Reuse Pi's native steering, terminal-tool results, and output caps. Keep coordinator and verifier execution unchanged, and distinguish estimated context from billed reasoning usage.
