...
Elenx uses Pi's context estimate, anchored to the latest applicable provider usage and estimated trailing messages. It uses Pi's default 16,384-token reserve for final notes and Pi's native output-cap helper, which includes its safety margin. The accumulated conversation stays intact. Requests ask for output caps that preserve this reserve. Providers that ignore output caps may overshoot the requested allocation, and context overflow remains an operational failure. The estimate is not cumulative reasoning usage or an exact measurement of hidden provider state.
