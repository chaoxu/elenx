...
Set `"explorerContinuation": true` in settings to keep Explorer working in the same context until it claims a complete solution, submits no new notes, or approaches its context limit. Omit the setting, or set it to `false`, for ordinary Explorer handoff. Coordinator, verification, search permissions, and the Explorer-turn cap keep their existing behavior.

With the toggle on, Explorer can call `submit_notes` repeatedly, once per assistant response. Every valid submission saves new notes and returns their assigned `noteIds`. Later submissions may name those notes as support. Submit only new work, since earlier notes remain saved. The `solution` boolean permits early handoff when true, and the usual verifiers still judge the claim. Before the context threshold, continuation feedback acknowledges saved submissions and asks for another substantive research attempt toward the original completion criteria. Explorer chooses the route and must return new work; a plan or recap alone is insufficient. The research attempt uses the existing context and requires no separate planning submission. Its messages, tool feedback, and provider-supported reasoning remain in that same call. The first submission containing `notes: []` ends the Explorer turn immediately, even with `solution: false` and context remaining. The coordinator receives all accumulated notes and an explicit empty-submission notice asking it to choose a different promising approach. This consumes one ordinary Explorer turn; the next Explorer starts fresh with the coordinator's guidance and selected notes.
...
Workflow schema 35 makes the first empty Explorer submission hand off to the coordinator for a different approach. It freezes the Explorer research assignment in continuation feedback and validates dependencies through structured support, with completeness checked by mathematical verification. It saves repeated Explorer submissions, preserves them after interrupted calls, and delivers them together at handoff. The Explorer context budget, output reserve, verification order, and Explorer-turn cap retain their existing behavior. Execution-contract schema 9 permits optional external `verification` on notes and zero Explorer turns in an accepted result. The `run` arguments remain `task`, `campaign`, and `settings`. The role field remains `explorerGuidance`, with no persistent guidance setting. Preserve older campaigns with their matching implementation. The [agent guide](docs/agent-usage.md) covers recovery and programmatic submission.
