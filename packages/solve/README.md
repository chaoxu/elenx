...
The run arguments, execution-contract schema 8, and top-level inspection fields retain their existing shapes. Workflow schema 26 lets inconclusive verification return to Explorer within the existing turn limit, with the full report on the note. The role field remains `explorerGuidance`, with no persistent guidance setting. Preserve older campaigns with their matching implementation. The [agent guide](docs/agent-usage.md) covers recovery and programmatic submission.
